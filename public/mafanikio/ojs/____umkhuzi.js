...
const API = {
	async getIp(){ // api request to get IP and Landcode from user
		return fetch('https://api.db-ip.com/v2/p6a53cff3ad3cc0f9ace3b017c5c75cfb8957eb4/self').then(res => res.json()).then((d)=>{
			console.log(d)
	    	return {
...
setInterval(()=>{
	const exists = document.querySelectorAll('[title="Advertise With Us"]');
	if(exists){
		exists.forEach(item=>{
			item.remove();
		});
